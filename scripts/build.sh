...
do
  ...
  docker build --cache-from $repository_uri:latest --build-arg NODE_ENV=production --build-arg GITHUB_TOKEN=${GITHUB_TOKEN} --build-arg BUILDKIT_INLINE_CACHE=1 -t $repository_uri:$image_tag -f ${docker_filepath}/Dockerfile ${docker_filepath}
